...
// GET /client/tasks/:taskId/messages - Paginated messages (for load more)
...
router.post('/tasks/:taskId/message', async (req, res) => {
  try {
    ...
    // - legacy strings: image URLs — behavior unchanged
    // - objects: R2 chat media metadata — fully server-validated
    if (attachments && attachments.length > 0) {
      ...
      for (let i = 0; i < attachments.length; i++) {
        ...
        if (typeof att === 'string') {
          ...
          // server-built metadata — never the client object as-is.
          try {
...
// =======================================================================
// CHAT MEDIA (Phase 1) — R2 direct-upload foundation (client)
// Media bytes NEVER pass through this server: browsers PUT directly to
// R2 using the short-lived presigned URLs issued below.
// =======================================================================

// POST /client/tasks/:taskId/media/upload-url — issue a presigned PUT URL
router.post('/tasks/:taskId/media/upload-url', async (req, res) => {
  try {
    ...
    const task = await Task.findById(taskId).select('clientId assignedUsers').exec();
    if (!task) {
      return res.status(404).json({ error: 'Task not found' });
    }
...
// GET /client/tasks/:taskId/media-url?key=... — short-lived presigned GET.
...
router.get('/tasks/:taskId/media-url', async (req, res) => {
  try {
    ...
    const { taskId } = req.params;
    const clientId = req.user.id; // identity from JWT only
...
router.post('/device-token', async (req, res) => {
  try {
    ...
    const userForPush = await User.findById(clientId).select('preferences').lean();
    if (userForPush?.preferences?.pushNotifications === false) {
      console.log(`[PUSH] Push notifications disabled in DB for client ${clientId} - skipping token registration`);
...
// PATCH /client/push-preference - Save push notification preference to database
router.patch('/push-preference', async (req, res) => {
  try {
    ...
    console.log(`[PUSH] Push preference saved for client ${clientId}: ${pushEnabled}`);
    return res.json({ success: true });
  } catch (err) {
