/**
 * One-Time Migration Script: Refresh Legal Pages
 *
 * Purpose: Updates existing legal page documents in the database with
 * the latest default content from the codebase. This is needed because
 * the seed function uses $setOnInsert which only applies on INSERT,
 * never on UPDATE — so existing documents with old "TaskFlow Pro"
 * content are never updated by the seed.
 *
 * This script uses $set (NOT $setOnInsert) to force-update content,
 * title, and metaDescription for all legal page slugs.
 *
 * It does NOT modify:
 * - isPublished (preserves admin publish/unpublish state)
 * - updatedBy (preserves admin attribution)
 * - The ensureDefaults $setOnInsert logic in LegalPage.js
 *
 * Usage: node src/scripts/refreshLegalPages.js
 *
 * After running, this script can be archived or deleted.
 */
...
async function run() {
  console.log('=== Legal Pages Migration Script ===');
  ...
  for (const page of defaults) {
    ...
    if (existing) {
      ...
      console.log(`  Title: ${existing.title} → ${result.title}`);
      console.log(`  Content length: ${(existing.content || '').length} → ${result.content.length} chars`);
      console.log(`  metaDescription: ${(existing.metaDescription || '').substring(0, 60)}...`);
    } else {
      created++;
  ...
  console.log(`Total processed: ${defaults.length} pages`);

  await mongoose.disconnect();
  console.log('\nDatabase disconnected. Safe to exit.');
}
