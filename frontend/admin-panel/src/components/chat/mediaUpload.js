...
// =====================================================================
// CHAT MEDIA (Phase 2B) — ADMIN-side media helpers
// Logic-identical to the audited client Phase 2A helpers; only the
// route prefix differs (/admin/tasks/... instead of /client/tasks/...).
// Centralized limits: single source of truth for UX validation.
// Server re-validates everything — this is UX-only.
// =====================================================================
...
export async function probeMediaEnabled(taskId) {
  try {
    await api.post(`/admin/tasks/${taskId}/media/upload-url`, { kind: 'probe' });
...
// In-memory cache with an expiry margin; 410 (expired) is cached
// negatively so we never hammer R2 for dead media. Never persisted.
// ---------------------------------------------------------------------
...
export async function fetchMediaUrl(taskId, key) {
  ...
  try {
    const res = await api.get(`/admin/tasks/${taskId}/media-url`, { params: { key } });
...
export async function deleteMedia(taskId, messageId, key) {
  await api.delete(`/admin/tasks/${taskId}/media`, { data: { messageId, key } });
}
