...
//   strictly as END DATE. There is no reliable completion timestamp in the data
//   model — no completedAt is invented and updatedAt is never used as one.
...
const WorkflowTimeline = () => {
  ...
  const timelineGraph = {};
  const summary = { orders: 0, starts: 0, ends: 0 };
        ...
        {loading ? (
          ...
          <>
            <p style={{ fontSize: '11px', fontWeight: '500', color: '#94a3b8', margin: '0 0 12px 0' }}>
              {summary.orders} order{summary.orders === 1 ? '' : 's'} placed · {summary.starts} task start{summary.starts === 1 ? '' : 's'} · {summary.ends} task end{summary.ends === 1 ? '' : 's'}
            </p>
                        ...
                        {timelineDays.map(day => {
                          ...
                          return (
                            <button
                              ...
                              title={`${dObj.getUTCDate()}: ${bars.filter(b => b.kind === 'order').length} orders · ${bars.filter(b => b.kind === 'start').length} starts · ${bars.filter(b => b.kind === 'end').length} ends`}
                        ...
                        <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', padding: '6px 0', borderBottom: '1px solid #f8fafc' }}>
                          ...
                          <div style={{ minWidth: 0, flex: 1 }}>
                            ...
                            <p style={{ fontSize: '11.5px', color: '#64748b', margin: 0 }}>
                              {ev.kind === 'start' ? 'Started' : 'End date'} · Status: {STATUS_META[ev.task.status]?.label || ev.task.status} · {(ev.task.creditCost || 0).toLocaleString('en-IN')} credits
                            </p>
