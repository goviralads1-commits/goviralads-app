...
// =====================================================================
// VoiceRecorder (Phase 2A) — browser-native MediaRecorder, NO dependency.
// Tap-to-record UX: tap mic -> recording (timer + Stop + Cancel) ->
// Stop produces the Blob which the parent uploads via the same
// direct-to-R2 flow as video. Hold-to-record is intentionally avoided.
// Parent renders this ONLY when MediaRecorder is supported.
// =====================================================================
...
const VoiceRecorder = ({ onRecorded, onCancel, onError }) => {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flex: 1, minWidth: 0, padding: '6px 10px', borderRadius: '20px', backgroundColor: '#fef2f2', border: '1px solid #fecaca', minHeight: '40px', boxSizing: 'border-box' }}>
      <span style={{ width: '10px', height: '10px', borderRadius: '50%', backgroundColor: '#ef4444', flexShrink: 0, animation: 'gva-rec-pulse 1.2s ease-in-out infinite' }} />
      <style>{'@keyframes gva-rec-pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.35; } }'}</style>
      ...
      <button onClick={handleCancel} title="Cancel" style={{ padding: '8px', borderRadius: '10px', border: 'none', backgroundColor: '#fff', cursor: 'pointer', display: 'flex', flexShrink: 0 }}>
      ...
      <button onClick={handleStop} title="Stop & send" style={{ padding: '8px 14px', borderRadius: '16px', border: 'none', backgroundColor: '#22c55e', color: '#fff', fontWeight: '600', fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 }}>
